...
# Primary: drpc (uncorrupted nonce state after mainnet.base.org 429 storm)
# Fallbacks ordered by reliability, mainnet.base.org last (rate-limited + nonce corruption)
BASE_RPC = 'https://base-mainnet.g.alchemy.com/v2/' + os.environ.get('ALCHEMY_API_KEY', '')
BASE_RPC_FALLBACK = [
    'https://base-mainnet.public.blastapi.io',
    'https://rpc.ankr.com/base',
    'https://base.blockpi.io/rpc/v1',
    'https://base.lavanet.xyz',
    'https://mainnet.base.org',  # rate-limited + nonce corruption after 429 storm
]
...
def buy_token(token_address: str, amount_usd: float, slippage: float = SLIPPAGE_DEFAULT) -> dict:
    ...
    try:
        ...
        usdc_cs = Web3.to_checksum_address(USDC_BASE)
        ...
        # Find the best Aerodrome pool for this token
        # If best pool is USDC (not WETH), use multi-hop: WETH -> USDC -> token
        pool_info = None
        try:
            sys.path.insert(0, str(Path(__file__).parent))
            from pool_resolver import get_pool_info, resolve_single
            pool_info = get_pool_info(token_cs)
            if not pool_info:
                # Cache miss — last-resort live DexScreener call
                pool_info = resolve_single(token_cs)
        except Exception as e:
            print(f'[EXECUTE] Pool lookup failed (non-fatal): {e}')

        if pool_info and pool_info.get('routing_token') == 'USDC':
            # Multi-hop: WETH -> USDC -> token
            routes = [
                {'from': weth_cs, 'to': usdc_cs, 'stable': True, 'factory': factory_cs},
                {'from': usdc_cs, 'to': token_cs, 'stable': False, 'factory': factory_cs},
            ]
            route_desc = f'WETH → USDC → {token_cs} (via Aerodrome USDC pool)'
            amount_out_min = 0  # will be set after getAmountsOut
        else:
            # Direct: WETH -> token (volatile pool)
            routes = [{
                'from': weth_cs,
                'to': token_cs,
                'stable': False,
        ...
        print(f'[EXECUTE] BUY {token_cs}: {route_desc} | spend {amount_eth:.6f} WETH, expect {expected_out} tokens, min_out {amount_out_min}')

        # Revert if no swap path (no liquidity or token not supported by router)
        if expected_out == 0:
            return {'status': 'failed', 'error': f'No swap path found for {token_cs} — check liquidity or router support'}
        ...
        if receipt['status'] == 1:
            ...
            # Record actual tokens received — balances may differ from pre-trade estimate
            try:
                decimals = token_contract.functions.decimals().call()
            except Exception:
                decimals = 18  # fallback for tokens without decimals()
            raw_balance = token_contract.functions.balanceOf(account.address).call()
            result['token_amount_out'] = raw_balance / (10 ** decimals)
...
def sell_token(token_address: str, sell_pct: float = 1.0, slippage: float = SLIPPAGE_DEFAULT) -> dict:
    ...
    try:
        ...
        # Revert if no swap path
        if expected_out == 0:
            return {'status': 'failed', 'error': f'No swap path found for {token_cs} on sell — check liquidity'}
