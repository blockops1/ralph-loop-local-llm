...
# Handle --critique and --review flags stub
CRITIQUE_MODE=0
REVIEW_MODE=0
if [ "$1" = "--critique" ]; then
    CRITIQUE_MODE=1
...
if [ "$CRITIQUE_MODE" = "1" ]; then
    ...
    OUTPUT_FILE="$PROJECTS_DIR/$SLUG/critique.md"
    $PYTHON - <<EOF
import json
import requests
...
# Review mode: snapshot production files, then run full codebase audit via ralph.py
if [ "$REVIEW_MODE" = "1" ]; then
    ...
    # Snapshot production files into code/ before review
    echo "[SYNC] Snapshotting production files for review..."
    $PYTHON "$SCRIPT_DIR/scripts/sync_review_files.py" "$SLUG"
    if [ $? -ne 0 ]; then
        echo "[SYNC] Failed — aborting review"
        exit 1
    fi
    echo "[REVIEW] Running Ralph review on snapshot..."
    # Load credentials
    ...
    LOG_FILE="$LOG_DIR/${SLUG}-review-$(date +%Y-%m-%dT%H%M%S).log"
    mkdir -p "$LOG_DIR"
    echo "=== Ralph Review: $SLUG ===" >> "$LOG_FILE"
    nohup $PYTHON "$RALPH_PY" "$SLUG" >> "$LOG_FILE" 2>&1 &
    RALPH_PID=$!
    echo "Ralph Review started: PID=$RALPH_PID log=$LOG_FILE"
    wait $RALPH_PID
    echo "Ralph Review finished: PID=$RALPH_PID exit=$?"
    exit 0
fi
...
# Auto-critique: skip for review-only projects (no git diff to critique)
if [ "${AUTO_CRITIQUE:-1}" != "0" ] && [ "$RALPH_EXIT" -eq 0 ]; then
    # Skip auto-critique for review-mode projects
    if grep -q '"type": *"review"' "$PROJECTS_DIR/$SLUG/prd.json" 2>/dev/null; then
        echo "[RALPH] Review story complete — skipping auto-critique"
    else
