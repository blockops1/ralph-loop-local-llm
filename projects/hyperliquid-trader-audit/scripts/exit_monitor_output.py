        ...
        if exit_reason:
            try:
                ...
                if result.get("status") == "success":
                    # Verify position is actually closed before marking as closed
                    remaining_positions = get_open_positions()
                    position_still_open = any(p.get("id") == position_id for p in remaining_positions)
                    ...
                    if not position_still_open:
                        ...
                        # Position still open despite successful sell response
                        actions.append({
                            "position_id": position_id,
                            "symbol": symbol,
                            "direction": direction,
                            "exit_reason": exit_reason,
                            "error": "position_still_open_after_sell",
                            "action": "failed"
                        })
                else:
                    actions.append({
