...
def get_next_story(prd: dict, max_attempts: int = 3) -> Optional[dict]:
    """
...
class PRDManager:
    """Object-oriented wrapper for PRD management functions."""
    ...
    def load(self) -> dict:
        """Load the PRD for this project."""
        self._prd = load_prd(self.slug)
        return self._prd

    def save(self, prd: dict) -> None:
        """Save the PRD to disk."""
        save_prd(prd, self.slug)

    def get_next_story(self, max_attempts: int = 3) -> Optional[dict]:
        """Get the next story to work on."""
        if self._prd is None:
            self.load()
        return get_next_story(self._prd, max_attempts)

    def mark_done(self, story_id: str, notes: str = "") -> dict:
        """Mark a story as done."""
        if self._prd is None:
            self.load()
        self._prd = mark_story_done(self._prd, story_id, notes)
        return self._prd

    def mark_failed(self, story_id: str, error: str = "") -> dict:
        """Mark a story as failed."""
        if self._prd is None:
            self.load()
        self._prd = mark_story_failed(self._prd, story_id, error)
        return self._prd
