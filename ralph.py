...
def build_system_prompt(story: dict, progress: str, agents_md: str) -> str:
    ...
    if story_type == "rework":
        template_path = RALPH_DIR / "PROMPT-rework.md"
    elif story_type == "review":
        template_path = RALPH_DIR / "PROMPT-review.md"
    else:
    ...
    if story_type == "rework":
        ...
        output_file = story.get("output_file", "")
        preserve = story.get("preserve", [])
        preserve_list = "\n".join(f"  - {p}" for p in preserve) if preserve else "(none specified)"
        current_behavior = story.get("current_behavior", "")
        desired_behavior = story.get("desired_behavior", "")
    ...
    while tool_call_count < max_tool_calls:
        ...
        for _attempt in range(3):
            try:
                ...
                err_str = str(e)
                # Retry on transient server errors (429=rate limit, 502/503/500=server issues)
                if any(code in err_str for code in ("429", "502", "503", "500")):
                    wait = 30 * (_attempt + 1)
                    log.warning(f"Model API {err_str[:80]} (attempt {_attempt+1}/3) - retrying in {wait}s")
                    time.sleep(wait)
                else:
...
def notify(msg: str, log: logging.Logger) -> None:
    """Send a Discord message to the trading reports channel via Bot API."""
    import os, re
        ...
        if env_path.exists():
            for line in env_path.read_text().splitlines():
                ...
                if m:
                    ...
                    break
    if not bot_token:
        log.warning("DISCORD_BOT_TOKEN not found - cannot notify")
        return
    ...
    try:
        resp = requests.post(
            ...
            json={"content": f"🐜 *Ralph:* {msg}"},
            timeout=10,
        )
        if resp.ok:
            log.info(f"Discord notification sent: {msg[:60]}")
        else:
